		...
		struct zap_details *details)
{
	...
	unsigned long end;
        ...
        struct mm_struct *mm = vma->vm_mm;
         /*
         * Cannnot unmap the primary region vma when direct segment is enabled
         * since we cannot free the physical pages on direct segment
         */
        if(mm->direct_segment_enabled == 1 && vma->is_primary_region == 1)
                return;
...
static int handle_pte_fault(struct vm_fault *vmf)
{
	...
	if (!vmf->pte) {
                if(vmf->vma->is_primary_region) 
                        printk("in primary Process handle_pte_fault:\n");
		if (vma_is_anonymous(vmf->vma))
