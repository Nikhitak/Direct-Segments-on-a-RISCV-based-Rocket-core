...
class TLB(instruction: Boolean, lgMaxSize: Int, nEntries: Int)(implicit edge: TLEdgeOut, p: Parameters) extends CoreModule()(p) {
  ...
  val ds_ppn = Reg(UInt(width = ppnBits))
  ...
  //NIKHITA DS logic begin
  val ds_base = io.ptw.dsb.base
  val ds_limit = io.ptw.dsl.limit
  val ds_range1 = Mux(lookup_tag >= ds_base, Bool(true), Bool(false))
  val ds_range2 = Mux(lookup_tag < ds_limit, Bool(true), Bool(false))
  when (io.req.valid && !tlb_miss && !hits(specialEntry)) {
    plru.access(OHToUInt(hits(normalEntries-1, 0)))
  }

  val ds_hit = !io.ptw.dsl.disable && ds_range1 && ds_range2 && tlb_miss
  if (usingVM) {
    when(!io.ptw.dsl.disable && ds_range1 && ds_range2 && tlb_miss) {
            ds_ppn := lookup_tag + io.ptw.dso.offset
      //      ds_hit := Bool(true)
    }
  }
  //NIKHITA DS logic end
  ...
  io.resp.paddr := Mux(ds_hit, Cat(ds_ppn, io.req.bits.vaddr(pgIdxBits-1, 0)), Cat(ppn, io.req.bits.vaddr(pgIdxBits-1, 0)))
  ...
  if (usingVM) {
    ...
    when (io.req.fire() && tlb_miss && !ds_hit) {
      printf(" it is a tlb miss for va %x\n", io.req.bits.vaddr)
      state := s_request
