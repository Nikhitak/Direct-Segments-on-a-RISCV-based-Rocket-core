...
class CSRFileIO(implicit p: Parameters) extends CoreBundle
  ...
  val dsb = new DSB().asOutput
  val dsl = new DSL().asOutput
  val dso = new DSO().asOutput
...
class CSRFile(perfEventSets: EventSets = new EventSets(Seq()))(implicit p: Parameters) extends CoreModule()(p)
  ...
  val reg_sdsb = Reg(new DSB)
  val reg_sdsl = Reg(new DSL)
  val reg_sdso = Reg(new DSO)
  ...
  if (usingVM) {
    ...
    read_mapping += CSRs.sdsb -> reg_sdsb.asUInt
    read_mapping += CSRs.sdsl -> reg_sdsl.asUInt
    read_mapping += CSRs.sdso -> reg_sdso.asUInt
  ...
  when (io.rw.cmd.isOneOf(CSR.S, CSR.C, CSR.W)) {
    ...
    if (usingVM) {
      ...
      when (decoded_addr(CSRs.sdsb))    { reg_sdsb.base := wdata(vpnBits-1,0) }
      when (decoded_addr(CSRs.sdsl))    { reg_sdsl.limit := wdata(vpnBits-1,0) }
      when (decoded_addr(CSRs.sdso))    { reg_sdso.offset := wdata(ppnBits-1,0) }
